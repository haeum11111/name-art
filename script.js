...
function analyzeSentiment(meaning) {
  ...
  if (text.includes('하나님') || text.includes('음성') || text.includes('새벽') || text.includes('빛') || text.includes('하늘') || text.includes('지키') || text.includes('신') || text.includes('우주') || text.includes('별') || text.includes('영혼') || text.includes('은혜')) {
    return { id: 1, name: '1. 너의 이름은 (은은한 네온 광원)' };
    ...
    return { id: 2, name: '2. 이탈리아 (클래식 단정 명조)' };
    ...
    return { id: 3, name: '3. 짱 (3D 입체 팝 블록)' };
    ...
    return { id: 4, name: '4. 꽃잎이 팔랑팔랑 (자연 캘리그라피)' };
    ...
    return { id: 5, name: '5. 너의 힘이 (굵은 외곽선 라운딩)' };
    ...
    return { id: 6, name: '6. 피터팬 (자유로운 붓터치)' };
    ...
    return { id: 7, name: '7. 술먹고갈래? (동글통통 버블)' };
    ...
    return { id: 8, name: '8. 라디오스타 (레트로 블록)' };
    ...
    return { id: 9, name: '9. 비판 (묵직한 3D 입체 섀도우)' };
    ...
    return { id: 10, name: '10. 날 (원형 엠블럼 타이포)' };
    ...
    return { id: 11, name: '11. 미샤 안심 (기하학 테이프 폴딩)' };
  } else {
    return { id: 12, name: '12. 개판오분전 (내추럴 펜 드로잉)' };
  }
}
...
function drawCard() {
  ...
  if (style.id === 1) {
    ...
    ctx.shadowBlur = 40;
    ctx.font = 'bold 150px "Sunflower", sans-serif';
    ctx.fillStyle = textColor;
    ctx.fillText(name, canvas.width / 2, canvas.height / 2);

  } else if (style.id === 2) {
    ctx.shadowBlur = 0;
    ctx.font = 'bold 130px "Song Myung", serif';
    ...
    ctx.shadowBlur = 0;
    ctx.font = '900 140px "Do Hyeon", sans-serif';
    for (let i = 18; i > 0; i--) {
    ...
    ctx.shadowBlur = 0;
    ctx.font = '140px "Nanum Pen Script", cursive';
    ctx.strokeStyle = strokeColor;
    ctx.lineWidth = 6;
    ctx.strokeText(name, canvas.width / 2, canvas.height / 2);
    ctx.fillStyle = textColor;
    ...
    ctx.shadowBlur = 0;
    ctx.font = 'bold 145px "Jua", sans-serif';
    ctx.strokeStyle = strokeColor;
    ctx.lineWidth = 22;
    ctx.lineJoin = 'round';
    ctx.strokeText(name, canvas.width / 2, canvas.height / 2);
    ...
    ctx.shadowBlur = 0;
    ctx.font = '170px "East Sea Dokdo", cursive';
    ctx.fillStyle = strokeColor;
    ...
    ctx.shadowBlur = 0;
    ctx.font = 'bold 150px "Jua", sans-serif';
    ctx.strokeStyle = strokeColor;
    ctx.lineWidth = 28;
    ctx.lineCap = 'round';
    ctx.strokeText(name, canvas.width / 2, canvas.height / 2);
    ...
    ctx.shadowBlur = 0;
    ctx.font = '900 140px "Black Han Sans", sans-serif';
    ctx.fillStyle = strokeColor;
    ...
    ctx.shadowBlur = 0;
    ctx.font = '900 145px "Do Hyeon", sans-serif';
    ctx.fillStyle = strokeColor;
    ...
    ctx.shadowBlur = 0;
    ctx.font = 'bold 125px "Gowun Batang", serif';
    ctx.strokeStyle = strokeColor;
    ctx.lineWidth = 12;
    ctx.strokeText(name, canvas.width / 2, canvas.height / 2);
    ctx.fillStyle = textColor;
    ...
    ctx.shadowBlur = 0;
    ctx.font = '120px "Nanum Pen Script", cursive';
    ctx.strokeStyle = strokeColor;
    ctx.lineWidth = 3;
    ctx.strokeText(name, canvas.width / 2, canvas.height / 2);
    ctx.fillStyle = textColor;
...
meaningInput.addEventListener('input', drawCard);
nameInput.addEventListener('input', drawCard);
textColorInput.addEventListener('input', drawCard);
strokeColorInput.addEventListener('input', drawCard);
