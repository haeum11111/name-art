...
// 한자 훈음(뜻과 음) 및 성향 키워드 감성 분류 알고리즘
function analyzeSentiment(meaning) {
  const t = meaning.toLowerCase();

  // 1. 은은한 네온 광원 (신성, 우주, 별, 빛, 은혜, 하늘, 신앙 및 관련 한자 훈음)
  const group1 = ['하나님', '음성', '새벽', '빛', '하늘', '지키', '신', '우주', '별', '영혼', '은혜', '신앙', '천사', '구원', '기도', '성령', '믿음', '은', '효', '가을하늘 민', '집 우', '하늘 우', '별 성', '은혜 은', '새벽 효'];
  
  // 2. 클래식 단정 명조 (지혜, 바름, 올곧음, 어짊, 슬기, 지성 및 관련 한자 훈음)
  const group2 = ['바른', '어질', '현명', '우아', '올곧', '정신', '깨끗', '마음', '고귀', '지혜', '단정', '지성', '슬기', '이치', '논리', '학자', '선비', '학문', '도덕', '예의', '지혜 지', '어질 현', '바를 정', '이치 리', '밝을 철'];
  
  // 3. 3D 입체 팝 블록 (최고, 짱, 주인공, 열정, 자신감, 준수 및 관련 한자 훈음)
  const group3 = ['최고', '짱', '당당', '화려', '열정', '에너지', '자신감', '주인공', '뛰어난', '빼어난', '준수', '준걸', '인기', '에이스', '리더', '중심', '목표', '성공', '준걸 준', '빼어날 수', '호걸 걸'];
  
  // 4. 자연 캘리그라피 (꽃, 숲, 바람, 이슬, 계절, 순수, 식물 및 관련 한자 훈음)
  const group4 = ['맑', '순수', '꽃', '자연', '숲', '바람', '향기', '봄', '이슬', '샘', '식물', '여름', '가을', '겨울', '산', '강', '바다', '초목', '나무', '잎', '여름 하', '향초 윤', '푸를 록', '꽃 화'];
  
  // 5. 굵은 외곽선 라운딩 (강함, 힘, 건강, 건설, 계승, 업적 및 관련 한자 훈음)
  const group5 = ['힘', '강', '용기', '키큰', '높은', '거대한', '든든', '건강', '우뚝', '세상', '세우', '건설', '업적', '계승', '기둥', '정복', '불굴', '승리', '세울 건', '공 훈', '잇을 승', '계승 승', '강할 강'];
  
  // 6. 자유로운 붓터치 (자유, 꿈, 도전, 파도, 날다, 솔직, 민첩 및 관련 한자 훈음)
  const group6 = ['자유', '꿈', '날아', '날다', '모험', '도전', '파도', '용맹', '민첩', '영리', '이끌', '솔직', '개척', '질주', '탐험', '날개', '비상', '민첩 민', '이끌 율', '솔직 율', '날 비', '물결 랑'];
  
  // 7. 동글통통 버블 (귀여움, 사랑, 예쁨, 아름다움, 통통, 애교 및 관련 한자 훈음)
  const group7 = ['귀여', '귀엽', '예쁜', '예쁘', '아름답', '사랑', '통통', '애교', '밝', '아기', '고울', '미소', '미인', '귀부인', '인형', '포근', '고울 연', '사랑 애', '아름다울 미'];
  
  // 8. 레트로 블록 (재미, 유쾌, 위트, 스타, 신나는, 개그, 방송 및 관련 한자 훈음)
  const group8 = ['재미', '즐겁', '유쾌', '위트', '신나는', '개그', '방송', '스타', '시작', '기원', '축제', '웃음', '해학', '엔터테인', '시작할 시', '일어날 흥', '즐길 락'];
  
  // 9. 묵직한 3D 입체 섀도우 (단단, 묵직, 정직, 신뢰, 진실, 바위 및 관련 한자 훈음)
  const group9 = ['단단', '묵직', '정직', '신뢰', '진실', '바위', '철학', '중후', '길', '도리', '공정', '신념', '성실', '책임', '약속', '길 도', '진실 윤', '공정 윤', '믿을 신', '열매 실'];
  
  // 10. 원형 엠블럼 타이포 (행복, 평화, 따뜻, 완성, 조화, 축복 및 관련 한자 훈음)
  const group10 = ['행복', '평화', '따뜻', '원형', '완성', '조화', '보듬', '상서', '축복', '안정', '화목', '결실', '나눔', '싸일 포', '상서로울 서', '화할 화', '복 복'];
  
  // 11. 기하학 테이프 폴딩 (특별, 세련, 트렌디, 디자인, 패션, 교양 및 관련 한자 훈음)
  const group11 = ['특별', '트렌디', '감각', '디자인', '세련', '패션', '스타일리시', '교양', '고상', '빛나는', '예술', '창의', '세련된', '우아할 아', '빛날 빈', '세련 빈', '빛날 채', '재주 예'];
